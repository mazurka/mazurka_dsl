...
scan_resource([#{name := <<"h1">>, children := NameList} = _RouteTag | Rest], Acc) ->
...
scan_mediatypes([], Acc) ->
  {lists:reverse(Acc), []};
scan_mediatypes([#{name := <<"h2">>, children := NameList} | Rest], Acc) ->
  Name = format_text(NameList, []),
...
scan_sections([#{name := <<"h3">>, children := NameList} | Rest], Acc, DefaultParser) ->
  Name = format_text(NameList, []),
  {Docs, Rest2} = scan_html(Rest, []),
  {Parser, Code, Rest3} = scan_code(Rest2, DefaultParser),
...
scan_code([#{name := <<"code">>, children := CodeList, info := Parser} | Rest], DefaultParser) ->
